...
(defplugin
  (:cmd
   ...
   (fn [{:keys [com bot nick channel args] :as com-m}]
     (when-privs com-m :admin (irc/mode com channel "+o" (first args)))))
  ...
  (:cmd
   ...
   #{"kick"}
   (fn [{:keys [com bot nick channel args] :as com-m}]
     (when-privs com-m :admin (irc/kick com channel (first args) :reason (apply str (rest args))))))

  (:cmd
   "Set's the channel's topic. ADMIN ONLY."
   #{"settopic"}
   (fn [{:keys [com bot nick channel args] :as com-m}]
     (when-privs com-m :admin (irc/set-topic com channel (apply str (interpose " " args))))))

  (:cmd
   "Ban's whatever you specify. ADMIN ONLY."
   #{"ban"}
   (fn [{:keys [com bot nick channel args] :as com-m}]
     (when-privs com-m :admin (irc/mode com channel "+b" (first args)))))
  ...
  (:cmd
   ...
   (fn [{:keys [com bot channel nick args] :as com-m}]
     (when-privs com-m :admin (irc/mode com channel "+v" (first args)))))
