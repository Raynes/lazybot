...
(defplugin
  (:fortune 
   ...
   (let [db (:fortunes (read-config fortunefile))]
     (if (zero? (count db))
       (send-message irc channel "I have no fortune cookies. Please feed me some!")
       (send-message irc channel (nth db (rand-int (count db)))))))
  ...
  (:addfortune 
   ...
   (if (seq args)
     (let [new-fortune (->> args (interpose " ") (apply str))
       ...
       (send-message irc channel "Fortune cookie eaten."))
     (send-message irc channel "An invisible fortune cookie?")))
