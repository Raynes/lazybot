...
(defplugin
  (:op 
   ...
   (if-admin nick irc-map (ircb/set-mode irc channel "+o" (first args))))
  ...
  (:kick 
   ...
   ["kick"] 
   [{:keys [irc nick channel args] :as irc-map}]
   (if-admin nick irc-map (ircb/kick irc channel (first args) :reason (apply str (rest args)))))

  (:settopic 
   "Set's the channel's topic. ADMIN ONLY."
   ["settopic"]
   [{:keys [irc nick channel args] :as irc-map}]
   (if-admin nick irc-map (ircb/set-topic irc channel (apply str (interpose " " args)))))

  (:ban 
   "Ban's whatever you specify. ADMIN ONLY."
   ["ban"]
   [{:keys [irc nick channel args] :as irc-map}]
   (if-admin nick irc-map (ircb/set-mode irc channel "+b" (first args))))
  ...
  (:voice 
   ...
   (if-admin nick irc-map (ircb/set-mode irc channel "+v" (first args))))
